...
Analysis of the Tanzmaus **bank dump** SysEx protocol (device → host bulk read of all pattern/song data), as documented by the [Windfisch/tanzmaus-reversing](https://github.com/Windfisch/tanzmaus-reversing) project.

> **Note:** this file only mirrors the remote repo's documentation. Findings from our own dumps/captures live in [reverse/factory-patterns.md](factory-patterns.md).
...
│   │───────│  │────────────│  │       │───────────│   │─────────│   │──────────│   │
│    MFB ID     tanzmaus ID    │       pattern no.    7-bit data    stripped     │
│                              │   (only when          payload     (see note)    │
│                        sequence counter    part==0)                            F7
```

byte   0:  F0            (SysEx begin)
bytes 1-3: 00 21 0B      (MFB manufacturer SysEx ID)
bytes 4-8: 04 00 03 00 00
byte   9:  part          (sequence counter 0..13=0x0D, then resets to 0)
byte  10:  pattern       (pattern no. 0..15, present only when part==0)
byte  11:  00
bytes 12..: payload      (7-bit encoded, see below)
last 4 bytes:            (stripped by `parse_tanzmaus_sysex`; the repo labels them "possibly a checksum")
...
`part` counts 0 → 13 and wraps; `pattern` increments whenever `part` wraps to 0. `parse_tanzmaus_sysex` in `util.py` verifies the sequence and lengths.

### 7-bit to 8-bit decoding

MIDI data bytes carry only 7 payload bits, so the raw payload is re-packed:

1. **7→8 pack**: take 8 bytes of 7 bits → concatenate 56 bits → re-split into 7 bytes of 8 bits (`convert_7to8` in `util.py`; each byte is fed with its low-7-bits reversed).
...
Decoded map (from the remote repo's `README.md`; yields 0xDA2 = 3490 bytes per pattern):

```
0000 - 01bf  step data: 16x 2-byte A pattern + 16x 2-byte B pattern,
              for BD, SD, RS, CP, TT, SP1, SP2 in that order (7 instruments)
01c0 - 01c7  bitmirrored "last step" values  (8 entries; the 8th is unexplained)
...
Per-instrument step data blocks (from the repo's bd/sd/rs/cp/tt/sp1/sp2 sections):
...
LFO pattern offsets: step data (80 vs 00) followed by 16x 2-byte data:

```
bdlfo 0a76  cplfo 0b16  ttlfo 0bb6  sp1lfo 0c56  sp2lfo 0cf6
```

### Encodings learned (from the remote repo)

- Accent: `(1,2,3,4,off)` = `(0fff, 0be8, 0800, 05dc, 0000)`
- Knob values are stored bit-reversed (verified via the "BD increasing tune" trace in the repo)
- `sampleOrAlt` = 00/01 for SP1, 02/03 for SP2
- `!!` (same position for SP1, 16/17 for SP2) is a sound index?

---

## Tool Usage (remote repo tools)
...
`util.py` provides `split_sysexes`, `convert_7to8`, `bitmirror`, `parse_tanzmaus_sysex`.

---

## Remote Repo's Outstanding Unknowns

- 1 unknown byte at `0d96`
- 7 always-zero(?) bytes
- 2 bytes at `0d9e-0d9f` that might be a checksum (or might not)
